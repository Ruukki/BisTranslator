...
/// </summary>
namespace BisTranslator.ClientData;
...
public class ClientUserInfo : IDisposable
{
    ...
    private unsafe void FrameworkOnUpdateInternal()
    {
        ...
        {
            _log.Debug($"[FrameworkUpdate]  Player is dead, returning");
            return;
        }
        ...
        if (_condition[ConditionFlag.BetweenAreas] || _condition[ConditionFlag.BetweenAreas51])
        {
            ...
            if (_lastZone != zone)
            {
                ...
                {
                    _log.Debug($"[ZoneSwitch]  Zone switch/Gpose start");
        ...
        if (_sentBetweenAreas)
        {
            _log.Debug($"[ZoneSwitch]  Zone switch/Gpose end");
        ...
        {
            _log.Error($"[FrameworkUpdate] Error during framework update of {ex}");
        }
        ...
        {
            _log.Debug($"Global draw block: END => {_lastGlobalBlockPlayer}");
            ...
            //     _log.Debug($"[FrameworkUpdate] GagSpeak Glamour Updates Finished");
            //     if(_config.disableGlamChangeEvent) {
            //         _log.Debug($"[FrameworkUpdate] Disabling Glamour Change Event");
            //         var lastLoggedOccurance = DateTime.Now;
            //         if ((lastLoggedOccurance - _lastLoggedTime).TotalMilliseconds < 100) {
            //             _log.Debug($"[FrameworkUpdate] Last logged occurance was less than 100ms ago, returning");
    ...
    public async Task WaitWhileCharacterIsDrawing(int timeOut = 5000, CancellationToken? ct = null)
    {
        ...
        _log.Debug($"[CharaDataHelper]  Waiting for character to finish drawing");
        ...
        try
        {
            ...
            _log.Debug($"[CharaDataHelper]  Finished drawing after {curWaitTime}ms");
        }
        catch (NullReferenceException ex)
        {
            _log.Warning($"[CharaDataHelper] Error accessing player pointer, they do not exist anymore {ex}");
        }
    ...
    {
        _log.Debug($"[Clear Async]  Running Clear Task");
        await Task.Delay(TimeSpan.FromSeconds(1), token).ConfigureAwait(false);
        _log.Debug($"[Clear Async]  Sending ClearCachedForObjectMessage");
    ...
    private bool IsBeingDrawn()
    {
        ...
        var curPtr = Address;
        _log.Information($"[CharaObjectHandler] IsBeingDrawn, CurPtr: {curPtr}, {this}, {curPtr.ToString()}");
        ...
        {
            _log.Information($"[CharaObjectHandler] IsBeingDrawn, CurPtr is ZERO, counter is {_ptrNullCounter}");
            _ptrNullCounter++;
            return true;
        }
        ...
        {
            _log.Information($"[CharaObjectHandler] IsBeingDrawn, CurPtr is ZERO, returning");
            Invalidate();
        ...
        _log.Information($"[CharaObjectHandler] IsBeingDrawn, DrawObjPtr: {drawObj.ToString()}");
        ...
        _log.Information($"[CharaObjectHandler] IsBeingDrawn, Condition: {isDrawn}");
    ...
    private unsafe bool CompareAndUpdateCustomizeData(byte* customizeData)
    {
        ...
        for (int i = 0; i < CustomizeData.Length; i++)
        {
            ...
            {
                _log.Debug($"[CompareAndUpdateCustomize]  has changes! {EquipSlotData[i]} to {data}");
    ...
    private unsafe bool CompareAndUpdateEquipByteData(byte* equipSlotData)
    {
        ...
        for (int i = 0; i < EquipSlotData.Length; i++)
        {
            ...
            {
                _log.Debug($"[CompareAndUpdateEquip]  has changes! {EquipSlotData[i]} to {data}");
                EquipSlotData[i] = data;
                hasChanges = true;
            }
        }
    ...
    private unsafe bool CompareAndUpdateMainHand(FFXIVClientStructs.FFXIV.Client.Graphics.Scene.Weapon* weapon)
    {
        ...
        if (hasChanges)
        {
            _log.Debug($"[CompareAndUpdateMainHand]  || Changed from modelsetid: " +
    ...
    private unsafe bool CompareAndUpdateOffHand(FFXIVClientStructs.FFXIV.Client.Graphics.Scene.Weapon* weapon)
    {
        ...
        if (hasChanges)
        {
            _log.Debug($"[CompareAndUpdateOffHand]  || Changed from modelsetid: " +
            $"{weapon->ModelSetId} to {MainHandData[0]} || variant: {weapon->Variant} to " +
            $"{MainHandData[1]} || secondaryid: {weapon->SecondaryId} to {MainHandData[2]}");
        }
        return hasChanges;
    ...
    private unsafe void CheckAndUpdateObject()
    {
        ...
        if (Address != IntPtr.Zero && DrawObjectAddress != IntPtr.Zero)
        {
            ...
            if (_clearCts != null)
            {
                _log.Debug($"[CheckAndUpdateObject] Cancelling Clear Task {this}");
                _clearCts.Cancel();
                _clearCts.Dispose();
                _clearCts = null;
            }
            ...
            && ((CharacterBase*)DrawObjectAddress)->GetModelType() == CharacterBase.ModelType.Human)
            {
                ...
                {
                    _log.Information($"[CHARA HANDLER UPDATE] classjob changed from {ClassJobId} to {classJob}");
        ...
        /*          This is not nessisary unless glamourer's IPC can fix
                ...
                if (equipDiff)
                    _log.Information($"[CheckAndUpdateObject] checking  equip data as game draw obj, result: {equipDiff}");
            }
            ...
            && ((CharacterBase*)DrawObjectAddress)->GetModelType() == CharacterBase.ModelType.Human) {
                ...
                if (customizeDiff)
                   _log.Information($"[CheckAndUpdateObject] Checking  customize data from game obj, result: {equipDiff}");
            }
            // if we had any different of any type, let the log know
            if (addrDiff || drawObjDiff || equipDiff || customizeDiff || nameChange) {
                _log.Information($"[CheckAndUpdateObject]  At least onne object changed this framework tick");
            }
        }
        */
    ...
    private unsafe void CheckCharacterForDrawing()
    {
        ...
        if (!IsAnythingDrawing)
        {
            ...
            if (isDrawingChanged)
            {
                _log.Debug($"Global draw block: START => {playerName} ({_lastGlobalBlockReason})");
            }
