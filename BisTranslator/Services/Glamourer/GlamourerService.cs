...
public sealed class GlamourerService
{
    ...
    public GlamourerService(DalamudPluginInterface pluginInterface, ClientUserInfo clientUserInfo, IPluginLog log)
    {
    ...
    private bool CheckGlamourerApiInternal()
    {
        ...
        finally
        {
            if (!apiAvailable)
            {
                _log.Error($"[GlamourerService]: Glamourer inactive. All Wardrobe functionality will not work.");
            }
        }
    }
    ...
    public async Task ApplyAllToCharacterAsync(string? customization, IntPtr character)
    {
        ...
        try
        {
            await _clientUserInfo.RunOnFrameworkThread(() =>
            {
                ...
                {
                    _log.Verbose("[ApplyAllAsyncIntrop] Calling on IPC: GlamourerApplyAll");
        ...
        {
            _log.Debug("[ApplyAllAsyncIntrop] Failed to apply Glamourer data");
        }
    }
    ...
    public async Task<string> GetCharacterCustomizationAsync(IntPtr character)
    {
        ...
        try
        {
            ...
            return await _clientUserInfo.RunOnFrameworkThread(() =>
            {
                ...
                if (gameObj is Character c)
                {
                    _log.Verbose("[GetCharacterCustomizationAsync] Calling on IPC: GlamourerGetAllCustomizationFromCharacter");
    ...
    public async Task SetItemToCharacterAsync(IntPtr character, byte slot, ulong item, byte dye, uint variant)
    {
        ...
        try
        {
            ...
            await _clientUserInfo.RunOnFrameworkThread(() =>
            {
                ...
                if (gameObj is Character c)
                {
                    _log.Verbose("[SetItemOnceToCharacterAsync] Calling on IPC: GlamourSetItemToCharacter");
        ...
        {
            // if at any point this errors, return an empty string as well.
            _log.Warning($"[SetItemOnceToCharacterAsync] Failed to set item to character with slot {slot}, item {item}, dye {dye}, and variant {variant}, {ex}");
            return;
        }
    }
        ...
        try
        {
            ...
            await _clientUserInfo.RunOnFrameworkThread(async () =>
            {
                try
                {
                    ...
                    if (gameObj is Character c)
                    {
                        _log.Verbose("[GlamourRevertIPC] Calling on IPC: GlamourerRevertToAutomationCharacter");
                        bool result = _RevertToAutomationCharacter.InvokeFunc(c, 1337);
                        _log.Verbose($"[GlamourRevertIPC] Revert to automation result: {result}");
                        if (!result)
                        {
                            _log.Warning($"[GlamourRevertIPC] Revert to automation failed, reverting to game instead");
                ...
                {
                    _log.Warning($"[GlamourRevertIPC] Error during GlamourerRevert: {ex}");
                }
            }).ConfigureAwait(false);
        }
        catch (Exception ex)
